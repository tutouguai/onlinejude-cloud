...
import mo.service.JudgeService;
...
//        String solution_id = UUID.randomUUID().toString().substring(0,12);
        int res = solutionMapper.insertOneItemIntoSolution(solution.getSolution_id(), solution.getProblem_id(),
                solution.getUser_id(),
...
//            if (problemMapper.addProblemOneSubmit(solution.getProblem_id()) > 0 && solutionMapper.insertCodeIntoSource(solution_id, sourceCode.getSource()) > 0) {
            if (solutionMapper.insertCodeIntoSource(solution.getSolution_id(), sourceCode.getSource()) > 0) {
                return true;
            } else {
    ...
    public int rejudge(String solutionId) {
        Solution solution = solutionMapper.getSolutionBySolutionId(solutionId);
        //查询数据库中的solution信息
        Integer judgeId = JudgeIdMap.getJudgeId(solution.getLanguage());
        //查询数据库中的的solution sourceCode
        String sourceCode = solutionMapper.getSourceCodeBySolutionId(solutionId);
        //查询数据库中problem中的时间与空间信息
        Problem problem = problemMapper.findProblemByProblemId(solution.getProblem_id());
        JudgeTask judgeTask = new JudgeTask(solution.getProblem_id(),problem.getTime_limit(),
                problem.getMemory_limit()*1024, judgeId, sourceCode);
        return judgeService.judge(judgeTask,solutionId);
    }
